...
public class HipiImageBundle {
  ...
  public final void openForWrite(boolean overwrite) throws IOException {
    ...
    if (fs.exists(indexFilePath) && !overwrite) {
      // 当数据库已经存在时，对数据进行追加，而不是报错
//      throw new IOException("HIB [" + indexFilePath.getName() + "] already exists. Cannot open HIB for writing unless overwrite is specified.");
      System.out.println("HIB [" + indexFilePath.getName() + "] already exists.");
      System.out.println("We will appned to the HIB [" + indexFilePath.getName() + "]");
    }
...
//      throw new IOException("HIB [" + dataFilePath.getName() + "] already exists. Cannot open HIB for writing unless overwrite is specified.");
      System.out.println("HIB [" + dataFilePath.getName() + "] already exists.");
      System.out.println("We will appned to the HIB [" + dataFilePath.getName() + "]");
    }
...
//        throw new IOException("Not implemented.");
        /// TODO: 以追加方式写入数据：当数据库已经存在时，对数据进行追加，而不是报错
        // 获取已有最新偏移量
        indexInputStream = new DataInputStream(fs.open(indexFilePath));
        while(indexInputStream.available() > 0) { // get current offet: 5374002
          currentOffset = indexInputStream.readLong();
        }
        indexInputStream.close();
        indexInputStream = null;
        System.out.println("current offset: " + currentOffset);
        // 初始化数据写入实例
        indexOutputStream = new DataOutputStream(fs.append(indexFilePath));
        dataOutputStream = new DataOutputStream(fs.append(dataFilePath));
      } else {
